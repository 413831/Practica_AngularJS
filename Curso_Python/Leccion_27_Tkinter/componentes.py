...
ventana = tk.Tk()
ventana.geometry('650x400+2000+400')
...
def crear_componentes_tabulador1(tabulador):
    ...
    boton1 = tk.Button(tabulador, text='Enviar', command=enviar)
...
def crear_componentes_tabulador3(tabulador):
    ...
    # Agregar un botón para saber qué opción seleccionó el usuario
    def mostrar_valor():
...
def crear_componentes_tabulador4(tabulador):
    imagen = tk.PhotoImage(file='pacman.png')
    def mostrar_titulo():
        messagebox.showinfo('Más info de imagen', f'Nombre imagen: {imagen.cget("file")}')
    # Imagen dentro de un boton
...
def crear_componentes_tabulador5(tabulador):
    ...
    def ejecutar_barra():
        ...
        for valor in range(101):
            # Esperemos un poco antes de continuar la ejecución
            sleep(0.05)
...
def crear_tabs():
    ...
    # Frame - agregamos un marco para agregar dentro del tab y organizar
    tabulador1 = ttk.Frame(control_tabulador)
    # Tabulador - agregamos el tabulador al control de tabuladores
